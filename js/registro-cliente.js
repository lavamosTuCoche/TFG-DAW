document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", (event) => {
      ...
      if (!validadorApellidos.test(apellidos)) {
        errores.push(new Error("Introduce al menos dos apellidos válidos. Se permiten letras y guiones."));
      }

      if (!validadorEmail.test(email) && !camposVacios) {
          errores.push(new Error("El email introducido no tiene una sintaxis válida"));
      }

      if (!validadorPassword.test(password) && !camposVacios) {
          errores.push(new Error("La contraseña no cumple los requisitos de seguridad (mínimo 10 caracteres, una mayúscula y un carácter especial)"));
      }
