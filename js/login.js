document.addEventListener("DOMContentLoaded", () => {
    ...
    const contenedorErrores =
        document.getElementsByClassName("container-errores")[0];
    ...
    form.addEventListener("submit", ($event) => {
        ...
        if (!email || !password) {
            errores.push(new Error("Rellena los campos del formulario de inicio de sesión"));
            camposVacios = true;
        }

        if (!email.match(validadorEmail) && !camposVacios) {
            errores.push(
                new Error("El email introducido no tiene una sintaxis correcta")
            );
        }
        ...
        if (!password.match(validadorPassword) && !camposVacios) {
            errores.push(
                new Error(
                    "La contraseña introducida no cumple con los requisitos de seguridad mínimos."
                )
            );
        }
